...
function $isValidModDir(dir) {
  const dirContent = ipcRenderer.sendSync('read-dir-sync', { path: `mods/${dir}` });
  return dirContent.indexOf(`${dir}.js`) >= 0 && dirContent.indexOf('Config.js') >= 0;
}
...
export default class ModManager extends Class {
   ...
   */
  async loadMods() {
    const promises = Promise.all(ipcRenderer
      .sendSync('read-dir-sync', { path: 'mods', onlyDirectories: true })
      .filter($isValidModDir)
      .map(modFolder => import(`../../../../mods/${modFolder}/${modFolder}.js`)),
    );
